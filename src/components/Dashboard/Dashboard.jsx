...
const Dashboard = () => {
  ...
  const [alertCounts, setAlertCounts] = useState({
    1: 0, // Pending PO Approval
    2: 0, // Pending Inprocess QC
    3: 0, // Pending Sales Return QC
    4: 0, // Bill Passing (Purchase)
    5: 0, // Bill Passing (Jobwork)
    6: 0, // Upcoming Dispatch
    7: 0, // Unauthorised BOM
    8: 0, // Pending Gate Inward Entry
  });
  ...
  const [stateWiseSalesData, setStateWiseSalesData] = useState([]);
  ...
  const fetchTotalRecordsData = async () => {
    ...
    try {
      const [
        ...
        safeFetch("http://127.0.0.1:8000/Sales/profoma-invoice/"),
  ...
  const fetchAlertCounts = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/Purchase/purchase-orders/unverified/simple/");
      if (res.ok) {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/Production/api/production-entries/");
      if (res.ok) {
        const json = await res.json();
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/Sales/get/sales-return/?start_date=2025-01-01&end_date=${todayStr}&cust_name=Ram%20kumawat`);
      if (res.ok) {
        const json = await res.json();
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/Account/purchase-po-date-filter/?from_date=2025-01-01&to_date=${todayStr}`, { headers });
      if (res.ok) {
        const json = await res.json();
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/Account/inwardchllan-date-fillter/?from_date=2025-01-01&to_date=${todayStr}`, { headers });
      if (res.ok) {
        const json = await res.json();
    ...
    // 6. Upcoming Dispatch
    try {
      const res = await fetch(`http://127.0.0.1:8000/Planning/dispatch-plan/?from_date=2025-01-01&to_date=${todayStr}`, { headers });
      if (res.ok) {
        const json = await res.json();
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/Store/api/gate-inward/");
      if (res.ok) {
        const json = await res.json();
